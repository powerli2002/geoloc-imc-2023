...
def get_traceroutes_results(traceroute_ids):
    ...
    while nb_tries > 0 and len(next_to_do) > 0:
        ...
        for id in to_do:
            try:
                ids = [id]
                traceroute_data = fetch_traceroutes_from_measurement_ids_no_csv(ids)
...
def get_circles_to_target(target_ip, vps=None):
    ...
    for probe in probes_data:
        ...
        if "address_v4" not in probe or probe["address_v4"] not in dict_rtt:
            continue
        if (
            "geometry" not in probe
            or "type" not in probe["geometry"]
            or probe["geometry"]["type"] != "Point"
            or "coordinates" not in probe["geometry"]
        ):
            continue
