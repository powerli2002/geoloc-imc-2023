...
def plot_multiple_error_bars(
    X, Ys, Yerrs, xmin, xmax, ymin, ymax, xlabel, ylabel, xscale, yscale, labels
):
    fig, ax = plt.subplots()
    ax.set_xlabel(xlabel, fontsize=fontsize_axis)
...
def plot_save(ofile: Path, is_tight_layout):
...
def plot_scatter_multiple(
    ...
    for i in range(0, len(Xs)):
        ...
        ax.scatter(X, Y, c=marker_colors[i], marker=markers[i], s=marker_size[i])
        # ax.plot(X, Y)
